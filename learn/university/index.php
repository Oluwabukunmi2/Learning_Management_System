...
if (isset($_POST['submit'])) {
	$ret = mysqli_query($bd, "SELECT * FROM university WHERE uemail='" . $_POST['uemail'] . "' and ( password='" . md5($_POST['password']) . "' and apstatus='approved')");
	...
	if ($num > 0) {
		...
		$status = 1;
		$log = mysqli_query($bd, "insert into universitylog(uid,username,userip,status) values('" . $_SESSION['id'] . "','" . $_SESSION['login'] . "','$uip','$status')");
		...
		$uip = $_SERVER['REMOTE_ADDR'];
		$status = 0;
		mysqli_query($bd, "insert into universitylog(username,userip,status) values('" . $_SESSION['login'] . "','$uip','$status')");
...
if (isset($_POST['change'])) {
	...
	$query = mysqli_query($bd, "SELECT * FROM university WHERE uemail='$uemail' and contactno='$contactno'");
	$num = mysqli_fetch_array($query);
	if ($num > 0) {
		mysqli_query($bd, "update university set password='$password' WHERE uemail='$uemail' and contactno='$contactno' ");
...
<head>
	...
	<title>E-Learn | University Login</title>
		...
		<div class="container">
			...
			<form class="form-login" name="login" method="post">
				...
				<div class="login-wrap">
					<input type="text" class="form-control" name="uemail" placeholder="University ID" required autofocus>
					<br>
